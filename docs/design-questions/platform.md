...
3. **Consolidate** — e.g. fold `agent-mcp.ts` into an `@doeixd/affe-ui-agent` adapter
...
§6 already says MCP is a "small `@doeixd/affe-ui-agent` adapter package", so a `src/`
...
`agent-mcp` folds into the `@doeixd/affe-ui-agent` adapter package (no `src/` module);
...
"explicitly out of library scope, or does `@doeixd/affe-ui-agent` ship an optional bridge
...
2. **Optional `@doeixd/affe-ui-agent` bridge, when demanded.** *Cost:* an adapter package
...
that if a bridge is ever built it belongs in `@doeixd/affe-ui-agent`, never in `src/`.
...
sentence recorded: an A2A bridge, if ever built, lives in `@doeixd/affe-ui-agent`,
never in `src/`.
</content>
</invoke>
