...
Prepared in discovery mode after AN-1–AN-4 and `@doeixd/affe-ui-agent` (AN-3) landed:
...
- **Recommendation (= the entry's own, sharpened):**
  ...
  - **Close the deferred naming:** ratify `src/ViewSpec.ts` (PascalCase
    ...
    stays out of `@doeixd/affe-ui-agent`: it has no adapter dependency and the specs
...
- **Context change:** `@doeixd/affe-ui-agent` now **exists** (AN-3), so the escape
...
- **Recommendation (= the entry's own):** **Option 1 with option 2 as the
  ...
  A2A bridge is ever built, it lives in `@doeixd/affe-ui-agent`, never in `src/`. The
