...
- `Behavior.attach(behavior, { select, merge? })` — **low-level/general** attach: `select` picks the behavior's elements from any bindings (including derived values like `() => bindings.filtered()`), which the contract-keyed forms can't. Prefer `attachToSlots` (authored) / `attachBySlotContract` (typed remap) / `attachBySlots` (dynamic string map) when a `View.Slots` contract exists; those are typed sugar over this.
