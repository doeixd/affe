...
| MCP server (`/mcp`, actions as tools, OAuth) | auto-mounted | Catalog → MCP tools is a pure projection: Effect Schema emits JSON Schema; descriptions live on catalog entries | Small `@doeixd/affe-ui-agent` adapter package |
...
- No SQL layer, no auth provider, no chat UI, no agent loop — app/adapter
  territory (`@doeixd/affe-ui-agent` adapter package, or userland). The library boundary
...
3. **AN-3 MCP projection** — `@doeixd/affe-ui-agent` mounts the catalog as an MCP server
...
5. ~~A2A/`ask-agent`: library scope or adapter?~~ **Decided (`DQ-098`,
   ...
   `@doeixd/affe-ui-agent`, never in `src/`.
