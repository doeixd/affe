...
import type { BenchmarkOwnership } from "../shared/browser-state.js";
    ...
    typeof entry !== "function"
      ...
      : () =>
        Effect.sync(() => {
          ...
          Effect.tap(() =>
            Effect.sync(() => {
              state.loaderResolvedAt = performance.now();
            })
          ),
        ),
  ]),
...
const installation = await Effect.runPromise(
  Resume.installClient({
    ...
    resolverEntries: trackedEntries,
    runtime,
...
function ownership(): BenchmarkOwnership {
...
window.__AF_BENCHMARK_WRITE_INDEPENDENT__ = (index, value) =>
  Effect.runPromise(
