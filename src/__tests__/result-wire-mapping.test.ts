 ...
 *
 * Promoted from future/result/wire-mapping.spec.ts (all green 2026-08-12), retyped.
 */

import { describe, expect, it, vi } from "vitest";
import { Result, type Result as ResultType } from "../effect-ts.js";
import { toWire, fromWire, type ResultWireValue, type SuccessWireValue } from "../result-wire.js";
import { encodeResult, decodeResult, encodeResultRecord } from "../Serialization.js";
...
describe("result-wire: the frozen §2.3 mapping", () => {
  it("[Slice 2/§2.3] encodes all 8 producible rows to the exact frozen bytes", async () => {
    const rows: ReadonlyArray<readonly [string, ResultType<unknown, unknown>, string]> = [
      // row 1
    ...
    const actual = await withClock(() =>
      rows.map(([name, core]) => [name, JSON.stringify(toWire(core))] as const),
    );
  ...
  it("[Slice 2/§2.3] decodes every row, including the two toWire cannot produce", async () => {
    const success = (value: unknown): SuccessWireValue => ({
      _tag: "Success" as const,
      value,
      waiting: false,
      timestamp: T,
    });
    const tagOf = (w: ResultWireValue) => {
      const r = fromWire(w);
  ...
  it("[Slice 2/§2.3 rows 5+10] pins rows 5 and 10 as LOSSY, so a silent fix is impossible", async () => {
    ...
    const stable: ReadonlyArray<ResultType<unknown, unknown>> = [
      Result.loading,
      Result.success(1),
    ...
    expect(stable.map((r) => fromWire(toWire(r))._tag)).toEqual([
      "Loading",
      "Success",
      "Refreshing",
      "Failure",
  ...
  it("[Decision 4] `timestamp` is write-only: clock-injected on encode, ignored on decode", async () => {
    ...
    const a = fromWire({ _tag: "Success", value: 1, waiting: false, timestamp: 1 });
    const b = fromWire({ _tag: "Success", value: 1, waiting: false, timestamp: 999 });
    expect(a).toEqual(b);
  ...
  it("[P15/§2.5] `Initial{waiting:false}` is UNREACHABLE on encode but already CLAIMED on decode — the slot is not free", async () => {
    ...
    const everyVariant: ReadonlyArray<ResultType<unknown, unknown>> = [
      Result.loading,
      Result.success(1),
      Result.refreshing(Result.success(1)),
    ...
    for (const r of everyVariant) {
      expect(toWire(r)).not.toEqual({ _tag: "Initial", waiting: false });
    }
    ...
    expect(fromWire({ _tag: "Initial", waiting: false })._tag).toBe("Idle");
    expect(fromWire({ _tag: "Initial", waiting: true })._tag).toBe("Loading");
    ...
    const { idle, isIdle, settled } = Result;
    ...
    expect(isIdle(fromWire({ _tag: "Initial", waiting: false }))).toBe(true);
    expect(fromWire({ _tag: "Initial", waiting: false })._tag).not.toBe("Loading");
