...
// Promoted from future/router/authoring-tiers.spec.ts (all green 2026-08-11).
...
describe("R3 — one authoring tier", () => {
  it("[R3] a failing unified-route guard prevents the loader from running and blocks navigation", async () => {
    ...
    const Guarded = Route.guard(Effect.suspend(() => {
      ...
      Route.loader((_params: {}) => Effect.sync(() => {
        loads += 1;
        return { ok: true };
      }))(
    ...
    const Open = Route.loader((_params: {}) => Effect.sync(() => {
      openLoads += 1;
      return { ok: true };
    ...
    await flush();
    const openSnapshot = Effect.runSync(runtime.snapshot());
  ...
  it("[R3] Component.route sugar produces a route with the same identity as Route.path", async () => {
    ...
    expect(Route.collectAll(Sugar).map((entry) => entry.meta.fullPattern))
      .toEqual(["/r3-tier/:id"]);

    // And it loads under the pattern identity, exactly like the canonical form.
    const sugarRun = await Effect.runPromise(
      Route.runMatchedLoaders(Sugar, new URL("http://test.local/r3-tier/alice")),
    );
    const canonicalRun = await Effect.runPromise(
      Route.runMatchedLoaders(Canonical, new URL("http://test.local/r3-tier-canonical/alice")),
    );
    expect(sugarRun.map((item) => item.routeId)).toEqual(["/r3-tier/:id"]);
    expect(canonicalRun.map((item) => item.routeId)).toEqual(["/r3-tier-canonical/:id"]);
    const first = sugarRun[0]?.result;
    expect(first?._tag).toBe("Success");
    if (first?._tag === "Success") {
      expect(first.value).toEqual({ from: "sugar", id: "alice" });
    }
  });
  ...
  it("[R3] Route.transition is deleted from the surface, while the wired features remain", async () => {
    ...
    // Absence is inherently untypeable, so it is asserted with `in` rather
    // than a property access that would need a cast to compile.
    expect("transition" in Route).toBe(false);
    ...
    expect("transition" in Route.Route).toBe(false);
  ...
  it("[R3] unified loaderErrorCases render the tagged fallback instead of throwing", async () => {
    ...
    const Page = Route.loaderError({
      ...
      Route.loader((_params: {}) => Effect.fail({ _tag: "NotFound" } as const))(
    ...
    const Ok = Route.loaderError({
      ...
      Route.loader((_params: {}) => Effect.succeed({ ok: true }))(
        Route.id("r3.errorcases.ok")(
          Route.path("/r3-error-ok/:id")(Component.from(() => "loaded")),
        ),
      ),
    );
    const okRendered = await Effect.runPromise(
